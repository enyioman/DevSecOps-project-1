pipeline {
    ...
    stages {
        ...
        stage('Docker Push') {
            steps {
                withVault(configuration: [skipSslVerification: true, timeout: 60, vaultCredentialId: 'jenkins-dockerr', vaultUrl: 'http://34.228.188.132:8200'], vaultSecrets: [[path: 'secrets/creds/docker', secretValues: [[vaultKey: 'username'], [vaultKey: 'password']]]]) {
                    sh "docker login -u ${username} -p ${password} "
                    sh 'docker push fynewily/sprint-boot-app:v1.$BUILD_ID'
                    sh 'docker push fynewily/sprint-boot-app:latest'
                    sh 'docker rmi fynewily/sprint-boot-app:v1.$BUILD_ID fynewily/sprint-boot-app:latest'
